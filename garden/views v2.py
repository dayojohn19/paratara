...
@api_view(['PUT'])
@csrf_exempt
def lookPlace(request, collectionStr=None):
    ...
    if collectionStr is not None:
        try:
            ...
            return HttpResponse(
                json.dumps({"error": f"Collection '{collectionStr}' not found."}),
...
def CreateQRCode(request, collectionObj, appDownloadLink):
    ...
    # qrImage = qrcode.make(f'{appDownloadLink} ,{json.dumps(collectionObj.forQRValues() )}')
    qrImage = qrcode.make(f'https://paratara.com/garden/home/{collectionObj.collectionUniqueID}')
    # http://127.0.0.1:8000/garden/home/asdasdasd
