...
def list_a4_collages(request):
    ...
    if request.method == 'GET':
        ...
        files = [f for f in os.listdir(a4_dir) if f.lower().endswith(('.png', '.jpg', '.jpeg','.svg'))]
...
def lookPlace(request, tokenStr=None,collectionStr=None):
    collection_key = collectionStr
    print('[lookPlace] Received request with collectionStr:', collectionStr)

@csrf_exempt
def look(request, collectionStr):
    # Check if API request (Accept: application/json)
    accept_header = request.headers.get('Accept', '')
    if not 'application/json' in accept_header:
        # Forward to lookPlace for API requests
        # print('[look] Detected API request, forwarding to lookPlace with collectionStr:', collectionStr)
        # return lookPlace(request, collectionStr=collectionStr)
        print('[look] Detected webpage request, serving SPA with collectionStr:', collectionStr)
        # Webpage request - serve Svelte SPA + assets
        from django.conf import settings
        import os
    ...
    if request.body:
        try:
            ...
            collection_key = bodyCollectionStr or collection_key
    ...
    if collection_key is not None:
        try:
            ...
            return HttpResponse(
                json.dumps({"error": f"we cannot find Collection, Please Scan QR."}),
    ...
    def serialize_collection_with_visitor(collection, visitor=None):
        serialized = collection.serialize()
        if visitor is not None:
            serialized['visitorCollections'] = [c.serialize() for c in visitor.visitorCollections.all()]
...
def registerAllImage(request):
    ...
    if cform.is_valid():
        ...
        # include_heading_title = collection_form.cleaned_data.get('include_heading_title', False)
        # include_qr_code = collection_form.cleaned_data.get('include_qr_code', False)
        # collection.collectionPicture = 'https://none.com'
        # print('Creating Code')
        # CreateQRCode(
        #     request,
        #     collection,
        ...
        for idx, img_url in enumerate(image_urls):
            ...
            # assign primary CollectionGroup FK so it's easy to query later
            collection.collectionGroup = collection_group
            # fallback if needed
            if not collection.collectionPicture:
                collection.collectionPicture = collection.collectionPlace.placePicture
            # Handle heading title and QR code options
            include_heading_title = collection_form.cleaned_data.get('include_heading_title', False)
            include_qr_code = collection_form.cleaned_data.get('include_qr_code', False)
            # You can use include_heading_title to control logic for heading title here if needed
            collection.save()
...
def CreateQRCode(request, collectionObj, appDownloadLink, customTitle="", include_heading_title=True, paste_qr=True):
    ...
    # --- NEW TITLE PLACEMENT LOGIC ---

    # Match title outline to margin color
    ...
    max_width = int(placeImage.width * 0.85)
    max_height = int(placeImage.height * 0.22)
    font_size = int(placeImage.height * 0.11)
    min_font_size = 18
    draw = ImageDraw.Draw(placeImage)
    while font_size >= min_font_size:
        font = load_font_with_fallback(font_path, font_size)
        bbox = draw.multiline_textbbox((0, 0), title_text, font=font, spacing=8)
        text_w = bbox[2] - bbox[0]
        text_h = bbox[3] - bbox[1]
   ...
   //////       ///
    ...
    # Save QR image as a standalone file
    qr_filename = f"group-{collectionObj.collectionGroup} place-{collectionObj.collectionPlace} {name_slug}-qr.png"
    qr_path = os.path.join(qr_dir, qr_filename)
    print(f"[CreateQRCode] Saving QR image to: {qr_path}")
    qrImage.save(qr_path, format='PNG')

    # Save only master image
    master_filename = f"group-{collectionObj.collectionGroup} place-{collectionObj.collectionPlace} {name_slug}-master.png"
