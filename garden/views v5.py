...
def lookPlace(request, tokenStr=None,collectionStr=None):
    ...
    print(f"[lookPlace] Set collectionCollector: {visitorUser}")
    collectionUser.collectionCollector = visitorUser
    collectionUser.collectionIsCollected = True
    print("[lookPlace] Marked collection as collected.")
    ...
    print(f"[lookPlace] Attempting to delete Cloudinary image with id or url: {localfile_id}")
    public_id = None
    if localfile_id and cloudinary:
        ...
        if localfile_id.startswith('http') or localfile_id.startswith('https'):
            ...
            if match:
                ...
                print(f"[lookPlace] Extracted public_id from URL: {public_id}")
            else:
                print(f"[lookPlace] Could not extract public_id from URL: {localfile_id}")
        else:
            public_id = localfile_id
        if public_id:
            try:
                result = cloudinary.uploader.destroy(public_id, invalidate=True)
                print(f"[lookPlace] Cloudinary delete result: {result}")
            except Exception as e:
                print(f"[lookPlace] Failed to delete Cloudinary image with public_id {public_id}: {e}")
        else:
            print(f"[lookPlace] No valid public_id found for Cloudinary deletion.")
    ...
    print(f"[lookPlace] Setting collectionLocalFile and collectionPicture to placePicture: {place_picture}")
    if place_picture:
        collectionUser.collectionLocalFile = place_picture
        collectionUser.collectionPicture = place_picture
        print(f"[lookPlace] Updated collectionLocalFile and collectionPicture to: {place_picture}")
    ...
    print(f"[lookPlace] Checking for Google Drive file to delete: {gdrive_url}")
    if gdrive_url and 'id=' in gdrive_url:
        ...
        if match:
            ...
            try:
                ...
                print(f"[lookPlace] Searching for MASTER file with query: {master_query}")
                master_files = service.files().list(q=master_query, fields='files(id, parents)').execute().get('files', [])
                print(f"[lookPlace] Found {len(master_files)} MASTER file(s) to delete.")
                for f in master_files:
                    # Optionally, check if parent is under Masters folder
                    print(f"[lookPlace] Deleting MASTER file: {master_name} ({f['id']})")
                    gdrive_upload.delete_gdrive_file_by_id(f['id'])
            except Exception as e:
                print(f"[lookPlace] Error checking/deleting QR/MASTER files: {e}")
            print(f"[lookPlace] Deleting original file_id: {file_id}")
            delete_gdrive_file_by_id(file_id)
    print("[lookPlace] Saving collectionUser and updating visitor collections.")
    collectionUser.save()
    visitorUser.visitorCollections.add(collectionUser)
    serializedData = collectionUser.serialize()
    print("[lookPlace] Returning serialized data.")
    return HttpResponse(json.dumps(serializedData), content_type="application/json")
...
# )
   ...
   //////       ///
    ...
    # Upload all 3 streams to Google Drive (no local files written)
    name_slug = f"{collectionObj.collectionName}-{collectionObj.collectionUniqueID}"
    print('\n\nUPLOADING TO GOOGLE DRIVE: .....')
    gdrive_url = ''
    try:
        from apis.gdrive_upload import upload_qr_card_images
        gdrive_url = upload_qr_card_images(
            qr_stream,
            master_stream,
            web_stream,
            name_slug,
            place_folder_name=collectionObj.collectionPlace.placeName,
            collection_folder_name=collectionObj.collectionName,
        )
        print('Uploaded to Google Drive:', gdrive_url, '\n\n\n')
    except Exception as e:
        print('Google Drive upload failed:', str(e))

    # Set the collection's Google Drive URL to the master Google Drive URL after upload
    if gdrive_url:
        collectionObj.collectionGoogleDriveURL = gdrive_url
    else:
        collectionObj.collectionGoogleDriveURL = collectionObj.collectionPicture
