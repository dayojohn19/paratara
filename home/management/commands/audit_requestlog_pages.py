...
class Command(BaseCommand):
    help = (
        "Check RequestLog.page values and count which would be 404 vs non-404. "
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--delete-404",
            action="store_true",
            help="Delete RequestLog rows whose page is detected as 404 (asks for confirmation unless --force).",
        )
        parser.add_argument(
            "--force",
            action="store_true",
        ...
        parser.add_argument(
            ...
            type=int,
            default=50,
            help="When using --delete-404, print up to N page values that will be deleted (default: 50; 0 to disable).",
        )
        parser.add_argument(
            "--delete-localhost",
            action="store_true",
            help="Delete RequestLog rows where ip_address is localhost (127.*). Uses confirmation unless --force.",
        )
    ...
    def handle(self, *args, **options):
        ...
        # Query is grouped in SQL for speed: we only iterate unique page values.
        grouped = (
            RequestLog.objects.values("page")
        ...
        raw_404_counts = Counter()  # raw page -> records referencing it
        ...
        for row in grouped.iterator(chunk_size=2000):
            ...
            # Cache key differs depending on how we check.
        ...
        total_records = RequestLog.objects.count()

        self.stdout.write(
            self.style.SUCCESS(
                f"=== RequestLog.page 404 check ({'HTTP' if use_http else 'resolver'} mode) ==="
            )
        )
        self.stdout.write(f"Total RequestLog records: {total_records}")
        self.stdout.write(f"Unique pages checked:     {len(cache_status)}")
        self.stdout.write("")
        ...
        if top_n > 0:
            self.stdout.write("")
            self.stdout.write(self.style.ERROR(f"-- Top {top_n} 404 pages (by RequestLog row count) --"))
    ...
    def _delete_404_rows(
        ...
        *,
        ...
        qs = RequestLog.objects.filter(page__in=raw_pages_to_delete)
        to_delete_count = qs.count()

        self.stdout.write("")
        self.stdout.write(self.style.WARNING("=== DELETE 404 REQUESTLOG ROWS ==="))
        self.stdout.write(f"Mode: {'HTTP' if use_http else 'resolver'}")
        self.stdout.write(f"Unique RequestLog.page values to delete: {len(raw_pages_to_delete)}")
        self.stdout.write(f"Total RequestLog rows to delete:         {to_delete_count}")

        if print_delete_limit != 0:
            limit = max(0, int(print_delete_limit))
            self.stdout.write("")
            self.stdout.write(self.style.WARNING("Will delete these page values (top by count):"))
            shown = 0
            for page, cnt in raw_404_counts.most_common():
                if page not in raw_pages_to_delete:
                    continue
                self.stdout.write(f"  {cnt:>7}  {page}")
                shown += 1
                if shown >= limit:
                    break
        ...
        if not force:
            confirm = input('Type "yes" to delete these RequestLog rows: ')
            if confirm.strip().lower() != "yes":
                self.stdout.write(self.style.ERROR("Deletion cancelled."))
                return

        deleted_count, _ = qs.delete()
        self.stdout.write(self.style.SUCCESS(f"Deleted {deleted_count} RequestLog rows."))
    ...
    def _delete_localhost_rows(self, *, force: bool, print_delete_limit: int) -> None:
        localhost_q = Q(ip_address__startswith="127.")
        qs = RequestLog.objects.filter(localhost_q)
        to_delete_count = qs.count()

        self.stdout.write("")
        self.stdout.write(self.style.WARNING("=== DELETE LOCALHOST REQUESTLOG ROWS ==="))
        self.stdout.write("Match: ip_address starts with 127.")
        self.stdout.write(f"Total RequestLog rows to delete:  {to_delete_count}")

        if to_delete_count == 0:
            self.stdout.write(self.style.WARNING("No localhost RequestLog rows found."))
            return
        ...
        if print_delete_limit != 0:
            ...
            grouped_local = (
                RequestLog.objects.filter(localhost_q)
                .values("page")
                .annotate(row_count=Count("id"))
                .order_by("-row_count")
            )
            ...
            for row in grouped_local.iterator(chunk_size=2000):
                self.stdout.write(f"  {int(row['row_count']):>7}  {row['page']}")
                shown += 1
                if shown >= limit:
                    break
        ...
        if not force:
            confirm = input('Type "yes" to delete these localhost RequestLog rows: ')
            if confirm.strip().lower() != "yes":
                self.stdout.write(self.style.ERROR("Deletion cancelled."))
                return

        deleted_count, _ = qs.delete()
        self.stdout.write(self.style.SUCCESS(f"Deleted {deleted_count} RequestLog rows (localhost)."))
