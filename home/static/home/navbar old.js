...
function SlowAddToPage(i) {
  setTimeout(function () {
    // console.log('Slow Adding To Page: ', i);
...
function AddToSearchBox(ListToAddInSearchBox) {
  setTimeout(() => {
    ...
    for (i in PlaceDataList) {
      newSearchItem = `<a class="dropdown-item search-drop-down" onclick="newTabPlaceSearch('${PlaceDataList[i].placename}');ChangeSearchInputValue('${PlaceDataList[i].placename}')" href="#about">${PlaceDataList[i].placename}</a>`;
...
class CreateObjectOnLandingPageOfCarpool {
  constructor(PlaceItemToCreateObject) {
    // console.log(' Creating Object For: ', PlaceItemToCreateObject);
    ...
    this.objectMainContainer.setAttribute("data-place", `${PlaceItemToCreateObject.placename}`);

    this.objectMainContainer.setAttribute("class", "container-item ");
    // background-image:  url(https://encrypted-tbn1.gstatic.com/images?q=tbn:ANd9GcQbPDHRJ1mpe3Ciqhm0quXXrKsDcz2TFBAJZrdC9aINdpgSwAxbdlGVr0n7KCOhIkfKc3kK3BTeeb-muzbqKNxrwqugqC0U0Rh7IhLxyg);
    // this.objectMainContainer.style.backgroundImage = `url(https://picsum.photos/id/${PlaceItemToCreateObject.id}/300/300)`;
    this.objectMainContainer.style.backgroundImage = `linear-gradient(to top, rgb(0, 0, 0), rgba(0, 0, 0, 0.2901960784313726) 15.55%, rgba(0, 0, 0, 0.3254901960784314) 20.17%, rgba(0, 0, 0, 0) 59.66%),url(${PlaceItemToCreateObject.placePhoto})`;
    ...
    this.objectSubContainer.innerHTML = `<p class="placenametitle">${PlaceItemToCreateObject.placename}</p>`;
