(function(){let config=window.placePageConfig||{},runWhenReady=window.placePageReady;window.ensurePlaceSubCardStyles=function(){if(document.getElementById(`pcs`))return;let style=document.createElement(`style`);style.id=`pcs`,style.textContent=`
      ...
      `}function setAuthStatus(form,message,isError){let status=form&&form.querySelector(`.paStatus`);status&&(status.textContent=message||``,status.style.color=isError?`#b91c1c`:`#64748b`)}function setAuthMode(mode,form){if(!form)return;let signup=mode===`signup`,contact=form.elements.contact,password=form.elements.password,switchButton=form.querySelector(`.paSwitch`);form.dataset.mode=signup?`signup`:`login`,contact.hidden=!signup,contact.required=signup,password.autocomplete=signup?`new-password`:`current-password`,signup?password.setAttribute(`minlength`,`6`):password.removeAttribute(`minlength`),form.querySelector(`.paTitle`).textContent=signup?`Create an account`:form.dataset.loginTitle||`Log in to rate`,form.querySelector(`.paSubmit`).textContent=signup?`Create account`:`Log in`,form.querySelector(`.paHint`).textContent=signup?`Already registered?`:`New here?`,switchButton.textContent=signup?`Log in`:`Create account`,switchButton.dataset.authMode=signup?`login`:`signup`,setAuthStatus(form,``,!1),(signup?contact:form.elements.username).focus()}function renderReviewPanel(item){let panel=document.getElementById(`prP`);if(!panel)return;let packageId=item&&item.packageId?String(item.packageId):``;if(rev.packageId=packageId,rev.commentsLoaded=!1,rev.loading=!1,rev.nextOffset=0,rev.hasMore=!1,!packageId){panel.hidden=!0,panel.innerHTML=``;return}let reviewForm=canReview?`
        ...
        `:authFormHtml(`Log in to rate`);panel.hidden=!1,panel.innerHTML=`
        <div class="prvs">
